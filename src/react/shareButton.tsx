...
export default function ShareButton(): React.JSX.Element {
    ...
    return <>
        {/*Share button*/}
        <button className="left-top-button icon-button" title="Share" onClick={() => {setShareOpen(true)}}>
        ...
        <dialog style={shareOpen ? {opacity: 1} : {display: "none", opacity: 0}} ref={shareDialogRef} onCancel={() => {setShareOpen(false)}}>
            ...
            <span className="dialog-text roboto-600" style={{marginTop: "5px"}}>Only people with RoAvatar can use this link</span>
        </dialog>
    </>
}